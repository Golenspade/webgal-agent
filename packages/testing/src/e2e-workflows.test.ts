...
intro:欢迎来到测试场景;
    ...
    const metaFiles = snapshots.filter((f) => f.includes(applyResult.snapshotId) && f.endsWith('.meta.json'));
...
runner.test('E2E: 幂等持久化 - 相同 idempotencyKey 返回相同 snapshotId，重启后仍有效', async () => {
  ...
  // First MCP instance
  {
    ...
    try {
      ...
      assertEqual(result2.snapshotId, firstSnapshotId, 'same idempotency key should return same snapshotId');
  ...
  // Second MCP instance (restart) - should still hit idempotency cache
  {
    ...
    try {
      ...
      assertEqual(result3.snapshotId, firstSnapshotId, 'after restart, same key should still return same snapshotId');

      // Verify file content is still the first one
      const fileContent = await callTool(client, 'read_file', {
        path: 'game/scene/idempotent_test.txt',
      });
      assertEqual(fileContent.content, 'First content', 'file should still have first content after restart');

      // Verify idempotency cache file exists
      const idemPath = path.join(projectRoot, '.webgal_agent', 'idem.json');
      const idemExists = await fs.access(idemPath).then(() => true).catch(() => false);
      ...
      assertEqual(idemData['test-key-123'].snapshotId, firstSnapshotId, 'cached snapshotId should match');
    } finally {
      client.kill();
      await cleanupTestProject(projectRoot);
    }
