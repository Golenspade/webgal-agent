...
runner.test('list_project_resources: should list all resources', async () => {
  ...
  try {
    ...
    assertOnlyKeys(
      result,
      ['backgrounds', 'figures', 'bgm', 'vocals', 'scenes'],
      'list_project_resources response',
    )
...
runner.test('list_project_resources: should filter by extension', async () => {
  ...
  try {
    ...
    // 背景应该只包含图片
    assert(
      result.backgrounds.every((f) => /\.(jpg|png|webp)$/i.test(f)),
      'backgrounds should be images',
    )

    // BGM 应该只包含音频
    assert(
      result.bgm.every((f) => /\.(mp3|ogg|wav)$/i.test(f)),
      'bgm should be audio',
    )

    // 场景应该只包含 .txt
    assert(
      result.scenes.every((f) => f.endsWith('.txt')),
      'scenes should be .txt',
    )
  } finally {
    await cleanupTestProject(projectRoot)
  }
...
runner.test(
  'list_project_resources: should return empty arrays for missing directories',
  async () => {
    const projectRoot = await createTestProject()
...
runner.test(
  'preview_scene: should return E_TOOL_DISABLED when executor not available',
  async () => {
    const projectRoot = await createTestProject()
...
runner.test('preview_scene: should timeout if no port found', async () => {
  ...
  try {
    ...
    await expectToolError(tools.previewScene({ scenePath: 'game/scene/start.txt' }), 'E_TIMEOUT')
  } finally {
    await cleanupTestProject(projectRoot)
  }
})
