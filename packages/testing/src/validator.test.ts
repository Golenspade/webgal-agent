...
runner.test('validate_script: should detect missing semicolons', async () => {
  ...
  try {
    ...
    assert(result.diagnostics.every(d => d.kind === 'syntax'), 'all should be syntax errors');
  } finally {
...
runner.test('validate_script: should detect unknown commands', async () => {
  ...
  try {
    ...
    assertEqual(result.valid, false);
    assert(result.diagnostics.some(d => d.kind === 'syntax' && d.message.includes('未知')), 'should have unknown command error');
  } finally {
    await cleanupTestProject(projectRoot);
  }
});
...
runner.test('validate_script: should not flag character dialogue as unknown command', async () => {
  ...
  try {
    ...
    // 角色台词不应被标记为未知指令
    const syntaxErrors = result.diagnostics.filter(d => d.kind === 'syntax' && d.message.includes('未知'));
...
runner.test('validate_script: should detect missing background', async () => {
  ...
  try {
    ...
    assertEqual(result.valid, false);
    assert(result.diagnostics.some(d => d.kind === 'resource'), 'should have resource error');
  } finally {
    await cleanupTestProject(projectRoot);
  }
});
