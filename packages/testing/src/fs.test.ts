...
runner.test('list_files: should support globs', async () => {
  ...
  try {
    ...
    assert(
      result.entries.some((f) => f.endsWith('.txt')),
...
runner.test('list_files: should return E_NOT_FOUND for missing directory', async () => {
  ...
  try {
    ...
    await expectToolError(tools.listFiles({ path: 'nonexistent' }), 'E_NOT_FOUND')
  } finally {
    await cleanupTestProject(projectRoot)
  }
})
...
runner.test('read_file: should respect maxBytes limit', async () => {
  ...
  try {
    ...
    await expectToolError(tools.readFile({ path: 'large.txt', maxBytes: 1000 }), 'E_TOO_LARGE')
  } finally {
    await cleanupTestProject(projectRoot)
  }
})
...
runner.test('read_file: should return E_NOT_FOUND for missing file', async () => {
  ...
  try {
    ...
    await expectToolError(tools.readFile({ path: 'nonexistent.txt' }), 'E_NOT_FOUND')
  } finally {
    await cleanupTestProject(projectRoot)
  }
})
...
runner.test('write_to_file: actual write should return snapshotId', async () => {
  ...
  try {
    ...
    assertOnlyKeys(
      result,
      ['applied', 'snapshotId', 'bytesWritten'],
      'write_to_file actual write response',
    )
...
runner.test('read_file: should enforce default sandbox.maxReadBytes', async () => {
  ...
  try {
    ...
    await expectToolError(tools.readFile({ path: 'big.txt' }), 'E_TOO_LARGE')
  } finally {
    await cleanupTestProject(projectRoot)
  }
})
