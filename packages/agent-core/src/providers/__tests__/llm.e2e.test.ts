...
function getConfigFromEnv(): LLMConfig | null {
  // 支持多种环境变量名称
  const apiKey = process.env.LLM_API_KEY || process.env.DEEPSEEK_API_KEY || process.env.OPENAI_API_KEY || process.env.ANTHROPIC_API_KEY;
...
describe('LLMProvider E2E Tests', () => {
  ...
  describe('真实 API 调用', () => {
    it('应该成功调用 LLM 并获取响应', async () => {
      ...
      if (response.usage) {
        console.log(`   Token 使用: 输入=${response.usage.inputTokens}, 输出=${response.usage.outputTokens}, 总计=${response.usage.totalTokens}`);
      }
    ...
    it('应该正确处理中文内容', async () => {
      ...
      const messages: LLMMessage[] = [
        { role: 'user', content: '用一个成语形容"非常开心"' },
      ];
  ...
  describe('错误处理', () => {
    it('应该处理无效的 API Key', async () => {
      ...
      await expect(
        provider.call([{ role: 'user', content: '测试' }])
      ).rejects.toThrow();
    }, 30000);
  });
});
