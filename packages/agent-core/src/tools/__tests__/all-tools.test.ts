...
`,
    ...
    // 创建测试资源文件
    await fs.writeFile(
      path.join(testProjectRoot, 'game', 'background', 'bg_beach.png'),
      'fake-png-data',
    )
    await fs.writeFile(
      path.join(testProjectRoot, 'game', 'background', 'bg_school.png'),
      'fake-png-data',
    )
    await fs.writeFile(
      path.join(testProjectRoot, 'game', 'figure', 'girl_happy.png'),
      'fake-png-data',
    )
  ...
  describe('1. 文件系统工具', () => {
    describe('1.1 list_files - 列出文件', () => {
      ...
      it('应该拒绝绝对路径', async () => {
        await expect(tools.listFiles({ path: '/etc/passwd' })).rejects.toMatchObject({
          error: { code: 'E_DENY_PATH' },
        })
      })

      it('应该拒绝路径逃逸', async () => {
        await expect(tools.listFiles({ path: '../../../etc' })).rejects.toMatchObject({
          error: { code: 'E_DENY_PATH' },
        })
      })
    })
    ...
    describe('1.2 read_file - 读取文件', () => {
      ...
      it('应该对不存在的文件返回错误', async () => {
        await expect(tools.readFile({ path: 'game/scene/nonexistent.txt' })).rejects.toMatchObject({
...
`,
  ...
  describe('6. 安全边界测试', () => {
    it('应该阻止访问 .git 目录', async () => {
      ...
      await expect(tools.readFile({ path: '.git/config' })).rejects.toMatchObject({
        error: { code: 'E_DENY_PATH' },
      })
    })

    it('应该阻止访问 node_modules 目录', async () => {
      await expect(tools.listFiles({ path: 'node_modules' })).rejects.toMatchObject({
        error: { code: 'E_DENY_PATH' },
      })
    })
    ...
    it('应该阻止路径遍历攻击', async () => {
      ...
      for (const maliciousPath of maliciousPaths) {
        await expect(tools.readFile({ path: maliciousPath })).rejects.toMatchObject({
