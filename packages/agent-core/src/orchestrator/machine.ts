...
export class Orchestrator {
  ...
  async run(
    ...
    try {
      ...
      const planResponse = await this.llm.call([{ role: 'user', content: planPrompt }])
      ...
      for (const [file, content] of this.generatedContent) {
        ...
        if (!validation.valid && validation.diagnostics.length > 0) {
          // 尝试自动修复
          const fixPrompt = this.promptBuilder.buildFixPrompt(content, validation.diagnostics)
      ...
      const writeConfirmed =
        (await callbacks?.onWriteConfirmation?.(this.currentPlan.scenes[0].file, {
          type: 'batch',
          scenes: this.currentPlan.scenes.length,
        })) ?? true
